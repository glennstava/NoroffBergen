...
INSERT INTO students (first_name, last_name, email) VALUES ('Marissa', 'Olsen', 'mari.ole@uib.no');
...
INSERT INTO orders (order_date, customer, ordered_item, quantity) VALUES
	('2022-12-05', 3, 4, 2);

SELECT ordered_item, quantity, quantity * 8590 AS 'total price' FROM orders WHERE customer = 3;
SELECT SUM(quantity * 8590) AS 'total price' FROM orders WHERE customer = 3;
SELECT customer, SUM(quantity * 8590) AS 'total price' FROM orders GROUP BY customer;
UPDATE PostOffice SET zipcode = 5005 WHERE zipcode = 5003;
